...
const company_details = function (req, res, next) {
  ...
  NSE_COMPANIES.findOne({ code: { $regex: pattern, $options: "mi" } })
    .select("-_id -__v")
    .then((result) => {
      if (result) {
        res.status(201).json({
    ...
    .catch((err) => {
      res
        .status(404)
        .json({ statusCode: 500, message: "error while fetching details" });
    });
};
...
const company_names = function (req, res, next) {
  ...
  // print(name);
  NSE_COMPANIES.find({ name: { $regex: name, $options: "i" } })
    .select("-_id -__v")
    .then((result) => {
      // print(result);
    ...
    .catch((err) => {
      res
        .status(404)
        .json({ statusCode: 404, message: "error while fetching" });
    });
};
